...
We first load the R packages needed for the exercise and set the working directory and open the dataset. 
...
We need to tell R about the survey design. In practice this often means specifying the units selected at the initial sampling stage ie primary sampling units as well as the strata. This is achieved with the `svydesign()` command. In effect this command creates a copy of the dataset with the survey design information attached, that can then subsequently be used for further estimation.
